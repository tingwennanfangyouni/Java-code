...
(function ($) {
    $.extend({
            ...
            importExcel: function(formId) {
            	...
            	layer.open({
            		...
            		btn1: function(index, layero){
                        ...
                        var okDate = layero.find('#okDate').val();
            			if (okDate == null || okDate == '' || okDate == undefined){
                            $.modal.msgWarning("请选择允许查看的日期。");
                            return false;
                        }
            			...
            			// 上传工资条时设置允许查看的日期
                        formData.append("okDate", $("input[name='okDate']").val());
            			$.ajax({
        ...
        tree: {
        	...
        	_lastValue: {},
        	// 初始化树结构
        	init: function(options) { //ztree初始化操作
        		...
        		var setting = {
    				callback: {
                        ...
                        onAsyncSuccess:options.onAsyncSuccess,               //初始化数完成 之后 的回调函数
        	    ...
        	    $.get(options.url, function(data) {
                    ...
                    //alert(node.tId + ", " + node.name + "," + node.checked);tree_1, 若依科技,false
